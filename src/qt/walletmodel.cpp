...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered mbmb address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 MBMB.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal mbmb:URI (mbmb:XyZ...?message=example)
            {
...
static void NotifyzMBMBReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzMBMBReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzMBMBReset.connect(boost::bind(NotifyzMBMBReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzMBMBReset.disconnect(boost::bind(NotifyzMBMBReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zmbmbTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
