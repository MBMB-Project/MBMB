...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::MBMB);
    ...
    if (!settings.contains("nAnonymizeMBMBAmount"))
        settings.setValue("nAnonymizeMBMBAmount", 1000);

    nAnonymizeMBMBAmount = settings.value("nAnonymizeMBMBAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeMBMBAmount"))
        SoftSetArg("-anonymizembmbamount", settings.value("nAnonymizeMBMBAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in mbmb.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMBMBAmount:
            return QVariant(nAnonymizeMBMBAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMBMBAmount:
            nAnonymizeMBMBAmount = value.toInt();
            settings.setValue("nAnonymizeMBMBAmount", nAnonymizeMBMBAmount);
            emit anonymizeMBMBAmountChanged(nAnonymizeMBMBAmount);
            break;
        case CoinControlFeatures:
