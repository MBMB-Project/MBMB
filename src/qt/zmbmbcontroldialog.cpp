...
#include "ui_zmbmbcontroldialog.h"
...
std::set<std::string> ZMbmbControlDialog::setSelectedMints;
std::set<CMintMeta> ZMbmbControlDialog::setMints;

ZMbmbControlDialog::ZMbmbControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZMbmbControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZMbmbControlDialog::~ZMbmbControlDialog()
{
    delete ui;
}

void ZMbmbControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZMbmbControlDialog::updateList()
{
...
void ZMbmbControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZMbmbControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZMbmb_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZMbmbControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZMbmbControlDialog::GetSelectedMints()
{
...
void ZMbmbControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
