...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "mbmb.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "mbmbd.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the MBMB and zMBMB money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-banscore=<n>", strprintf(_("Threshold for disconnecting peers (default: %u)"), 100));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 2119, 51474));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in MBMB/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in MBMB/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, mbmb, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in MBMB/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all MBMB specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-mbmbstake=<n>", strprintf(_("Enable or disable staking functionality for MBMB inputs (0-1, default: %u)"), 1));
    //strUsage += HelpMessageOpt("-zmbmbstake=<n>", strprintf(_("Enable or disable staking functionality for zMBMB inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:2119"));
...
//#ifdef ENABLE_WALLET
    ...
    //strUsage += HelpMessageOpt("-backupzmbmb=<n>", strprintf(_("Enable automatic wallet backups triggered after each zMBMB minting (0-1, default: %u)"), 1));
    //strUsage += HelpMessageOpt("-zmbmbbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zMBMB backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
//#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizembmbamount=<n>     " + strprintf(_("Keep N MBMB anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 2118, 51475));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) %i The MBMB Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that MBMB is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. MBMB Core is shutting down."));
...
#endif
    // Make sure only a single MBMB process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. MBMB Core is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //MBMB: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //MBMB specific: zerocoin and spork DB's
                ...
                // MBMB: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZMBMBMinted();
                        RecalculateZMBMBSpent();
                    }
                    RecalculateMBMBSupply(1);
                }
                ...
                if (GetBoolArg("-reindexaccumulators", false)) {
                    if (chainActive.Height() > Params().Zerocoin_Block_V2_Start()) {
                        ...
                        // MBMB: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of MBMB Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart MBMB Core to complete") << "\n";
        ...
        zwalletMain = new CzMBMBWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zMBMBWallet
        //uiInterface.InitMessage(_("Syncing zMBMB wallet..."));

        //bool fEnableZMbmbBackups = GetBoolArg("-backupzmbmb", true);
        //pwalletMain->setZMbmbAutoBackups(fEnableZMbmbBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zmbmbTracker->Init();
...
//    }
//
//    nAnonymizeMBMBAmount = GetArg("-anonymizembmbamount", 0);
//    if (nAnonymizeMBMBAmount > 999999) nAnonymizeMBMBAmount = 999999;
//    if (nAnonymizeMBMBAmount < 2) nAnonymizeMBMBAmount = 2;
    ...
    LogPrintf("Anonymize MBMB Amount %d\n", nAnonymizeMBMBAmount);
    ...
    /* Denominations
       ...
       For example:
       1MBMB+1000 == (.1MBMB+100)*10
       10MBMB+10000 == (1MBMB+1000)*10
    */
