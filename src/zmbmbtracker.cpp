...
CzMBMBTracker::CzMBMBTracker(std::string strWalletFile)
{
...
CzMBMBTracker::~CzMBMBTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzMBMBTracker::Init()
{
...
bool CzMBMBTracker::Archive(CMintMeta& meta)
{
...
bool CzMBMBTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzMBMBTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzMBMBTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzMBMBTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzMBMBTracker::GetSerialHashes()
{
...
CAmount CzMBMBTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzMBMBTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzMBMBTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzMBMBTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzMBMBTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzMBMBTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzMBMBTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzMBMBTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzMBMBTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzMBMBTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzMBMBTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzMBMBTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzMBMBTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzMBMBTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzMBMBTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzMBMBTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzMBMBTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzmbmb from DB\n", __func__, listDeterministicDB.size());
    }
