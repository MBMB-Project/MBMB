...
// Copyright (c) 2016-2018 The MBMB developers
...
public:
    ...
    bool WriteZMBMBSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZMBMBSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZMBMBSeed_deprecated(uint256& seed);
    bool EraseZMBMBSeed();
    bool EraseZMBMBSeed_deprecated();

    bool WriteZMBMBCount(const uint32_t& nCount);
    bool ReadZMBMBCount(uint32_t& nCount);
