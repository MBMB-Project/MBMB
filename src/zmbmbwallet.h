...
#ifndef MBMB_ZMBMBWALLET_H
#define MBMB_ZMBMBWALLET_H
...
public:
    CzMBMBWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZMBMB(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZMBMB(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //MBMB_ZMBMBWALLET_H
