...
CzMBMBWallet::CzMBMBWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zmbmb seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZMBMBSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZMBMBSeed_deprecated()) {
                    LogPrintf("%s: Updated zMBMB seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zmbmb seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zmbmb wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzMBMBWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZMBMBCount(nCountLastUsed);
    else if (!walletdb.ReadZMBMBCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzMBMBWallet::Lock()
{
    seedMaster = 0;
}

void CzMBMBWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzMBMBWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZMBMB(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzMBMBWallet::LoadMintPoolFromDB()
{
...
void CzMBMBWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzMBMBWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzMBMBWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zmbmbTracker->HasPubcoinHash(pMint.first)) {
...
bool CzMBMBWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZMBMB(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zmbmbTracker which also adds to database
    pwalletMain->zmbmbTracker->Add(dMint, true);
...
void CzMBMBWallet::SeedToZMBMB(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzMBMBWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzMBMBWallet::GenerateDeterministicZMBMB(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzMBMBWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzMBMBWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
